...
const CreatePin = ({ user }) => {
  ...
  /**
   * Method to handle the changes in image field.
   * @param {object} e 
   */
  const uploadImage = (e) => {
    const { type, name } = e.target.files[0];

    if(type === 'image/png' || type === 'image/jpg' || type === 'image/jpeg' || type === 'image/svg' || type === 'image/gif' || type === 'image/tiff') {
